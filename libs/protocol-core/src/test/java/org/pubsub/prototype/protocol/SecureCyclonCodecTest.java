...
class SecureCyclonCodecTest {
    @Test void roundTripsExchangesAndAdvertisedEndpoint() {
        var peer = new NodeEndpoint("a".repeat(64), "node-1", 7000);
        var exchange = new GossipExchange(List.of(NodeDescriptor.fresh(peer, 100).transfer("b".repeat(64))), List.of());
        ...
        var hello = ProtocolMessage.hello(new NodeId(peer.nodeId()), "node-1").withEndpoint(peer);
        assertEquals(hello, codec.decode(codec.encode(hello)));
    }
    @Test void rejectsMissingBodyInvalidEndpointAndWrongVersion() {
        var codec = new ProtocolCodec();
        assertThrows(IllegalArgumentException.class,
                () -> ProtocolMessage.gossip(MessageType.SECURECYCLON_REQUEST, UUID.randomUUID(), null));
        assertThrows(ProtocolException.class, () -> codec.decode("{\"type\":\"SECURECYCLON_REQUEST\",\"version\":99}".getBytes()));
        assertThrows(IllegalArgumentException.class, () -> new NodeEndpoint("a".repeat(64), "0.0.0.0", 7000));
        assertThrows(IllegalArgumentException.class, () -> new NodeEndpoint("a".repeat(64), "node", 65536));
    }
}
